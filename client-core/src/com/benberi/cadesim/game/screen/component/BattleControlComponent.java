...
public class BattleControlComponent implements InputProcessor {
	...
	public void initListeners() {
		disengageButton.addListener(new ClickListener() {
    		...
    		public void clicked(InputEvent event, float x, float y) {
    			context.sendDisengageRequestPacket();
    		}
    	});
    	chatScroller.addListener(new InputListener() {
    		public void enter(InputEvent event, float x, float y, int pointer, Actor actor) {
    			stage.setScrollFocus(chatScroller);
    		}
        ...
        if (move.getMove() == MoveType.NONE) {
            if (button == Input.Buttons.LEFT) {
                if (leftMoves > 0) {
                    ...
                    context.sendSelectMoveSlot(position, MoveType.LEFT);
                }
                else if (forwardMoves > 0) {
                    placeMove(position, MoveType.FORWARD, true);
                    context.sendSelectMoveSlot(position, MoveType.FORWARD);
                }
                else if (rightMoves > 0) {
                    placeMove(position, MoveType.RIGHT, true);
                    context.sendSelectMoveSlot(position, MoveType.RIGHT);
                }
            }
            ...
            if (button == Input.Buttons.LEFT) {
                ...
                if (hasMove(next)) {
                    ...
                    context.sendSelectMoveSlot(position, next);
                }
                else if (hasMove(next.getNext())) {
                    placeMove(position, next.getNext(), true);
                    context.sendSelectMoveSlot(position, next.getNext());
                }
                else if (hasMove(next.getNext().getNext())) {
                    placeMove(position, next.getNext().getNext(), true);
                    context.sendSelectMoveSlot(position, next.getNext().getNext());
                }
            }
            else if (button == Input.Buttons.RIGHT) {
    ...
    private void renderMoveControl() {
        ...
        stage.getBatch().draw(title, MOVES_titleX, MOVES_titleY);
        if (isDragging && startDragSlot != -1) {
            ...
            if ((startDragMove != MoveType.NONE) || (isBigShip && (startDragSlot == blockingMoveSlot))) {
                stage.getBatch().draw(t, draggingPosition.x - t.getRegionWidth() / 2, Gdx.graphics.getHeight() - draggingPosition.y - t.getRegionHeight() / 2);
            }
        }
    ...
    private void renderDisengage() {
    	stage.getBatch().begin();
    	stage.getBatch().draw(disengageBackground, DISENGAGE_backgroundX, DISENGAGE_backgroundY);
    	stage.getBatch().end();
    }
    
    private void drawMoveHolder() {
        // The hand bg
        stage.getBatch().draw(shiphand, MOVES_shiphandX, MOVES_shiphandY);
        ...
        for (int i = 0; i < movesHolder.length; i++) {
            ...
            stage.getBatch().draw((left[0])?cannonLeft:emptyCannonLeft, MOVES_cannonLeftSlotSmallX, cH); // left
            if (isDoubleShot) {
                stage.getBatch().draw((left[0] && left[1])?cannonLeft:emptyCannonLeft, MOVES_cannonLeftSlotBigX, cH); // left
            }
            ...
            if (isDoubleShot)
            {
                stage.getBatch().draw((right[0])?cannonRight:emptyCannonRight, MOVES_cannonRightSlotSmallX, cH); // right
                stage.getBatch().draw((right[0] && right[1])?cannonRight:emptyCannonRight, MOVES_cannonRightSlotBigX, cH); // right        
            }
            else
            {
            	stage.getBatch().draw((right[0])?cannonRight:emptyCannonRight, MOVES_cannonRightSlotSmallX, cH); // right
            ...
            if (isBigShip && (i == blockingMoveSlot)) {
                stage.getBatch().draw(blockingMoveTexture, MOVES_moveSlotX, mH);
            }
            else
            {
                if (move.getMove() != MoveType.NONE) {
                    ...
                    stage.getBatch().draw(getTextureForMove(move.getMove()), MOVES_moveSlotX, mH);
                    stage.getBatch().setColor(Color.WHITE);
                }
            }
        }
	...
	private void drawMovesSelect() {
        // auto, cannons
        stage.getBatch().draw(autoBackground, MOVES_autoBackgroundX, MOVES_autoBackgroundY);
        font.draw(stage.getBatch(), "Auto", MOVES_autoTextX, MOVES_autoTextY);
        stage.getBatch().draw(auto?autoOn:autoOff,            MOVES_autoX,    MOVES_autoY);
        stage.getBatch().draw((cannons>0)?cannon:emptyCannon, MOVES_cannonsX, MOVES_cannonsY);
        font.draw(stage.getBatch(), "x" + Integer.toString(cannons), MOVES_cannonsTextX, MOVES_cannonsTextY);

        // moves
        stage.getBatch().draw((leftMoves == 0)?emptyLeftMoveTexture:leftMoveTexture, MOVES_leftX, MOVES_leftY);
        stage.getBatch().draw((forwardMoves == 0)?emptyForwardMoveTexture:forwardMoveTexture, MOVES_forwardX, MOVES_forwardY);
        stage.getBatch().draw((rightMoves == 0)?emptyRightMoveTexture:rightMoveTexture,  MOVES_rightX, MOVES_rightY);
        // radios
        Texture onTex  = auto?radioOnDisable:radioOn;
        Texture offTex = auto?radioOffDisable:radioOff;
        stage.getBatch().draw(radioButtons[0]?onTex:offTex, MOVES_leftRadioX,    MOVES_leftRadioY);
        stage.getBatch().draw(radioButtons[1]?onTex:offTex, MOVES_forwardRadioX, MOVES_forwardRadioY);
        stage.getBatch().draw(radioButtons[2]?onTex:offTex, MOVES_rightRadioX,   MOVES_rightRadioY);
        ...
        if (targetMove != null) {
            switch(targetMove) {
                case LEFT:
                    stage.getBatch().draw(sel, MOVES_leftSelectX, MOVES_leftSelectY);
                    enableRadio(0);
                    break;
                case FORWARD:
                    stage.getBatch().draw(sel, MOVES_forwardSelectX, MOVES_forwardSelectY);
                    enableRadio(1);
                    break;
                case RIGHT:
                    stage.getBatch().draw(sel, MOVES_rightSelectX, MOVES_rightSelectY);
                    enableRadio(2);
                    break;
            }
        }
        ...
        font.draw(stage.getBatch(), "x" + Integer.toString(leftMoves   ), MOVES_leftMovesTextX,    MOVES_leftMovesTextY);
        font.draw(stage.getBatch(), "x" + Integer.toString(forwardMoves), MOVES_forwardMovesTextX, MOVES_forwardMovesTextY);
        font.draw(stage.getBatch(), "x" + Integer.toString(rightMoves  ), MOVES_rightMovesTextX,   MOVES_rightMovesTextY);
    }
    ...
    private void drawTimer() {
        stage.getBatch().draw(hourGlass, MOVES_hourGlassX, MOVES_hourGlassY);
        stage.getBatch().draw(sandTrickle,MOVES_sandTrickleX, MOVES_sandTrickleY );
        stage.getBatch().draw(sandTop, MOVES_sandTopX, MOVES_sandTopY);
        stage.getBatch().draw(sandBottom, MOVES_sandBottomX, MOVES_sandBottomY);
    }
    ...
    private void drawShipStatus() {
        stage.getBatch().draw(shipStatusBg, MOVES_shipStatusBackgroundX, MOVES_shipStatusBackgroundY);
        ...
        stage.getBatch().draw(shipStatus, MOVES_shipStatusX, MOVES_shipStatusY);
    }
    ...
    public void resetPlacedMovesAfterTurn() {
        for (int i = 0; i < movesHolder.length; i++) {
            ...
            context.sendSelectMoveSlot(i, MoveType.NONE);
        }

        // reset slider
        if (isBigShip) {
	        blockingMoveSlot = 3;
	        context.sendBlockingMoveSlotChanged(3);
        }
    }
        ...
        for (int i=0; i<4; i++) {
            ...
            if (left > 0) {
                for (int j=0; j<(rolloverThreshold - left); j++) {
                    context.sendAddCannon(0, i);
                }
                setCannons(0, i, 0);
            }
            if (right > 0) {
                for (int j=0; j<(rolloverThreshold - right); j++) {
                    context.sendAddCannon(1, i);
                }
                setCannons(1, i, 0);
            }
        }
    ...
    public boolean touchUp(int x, int y, int pointer, int button) {
    	boolean controlsLocked = context.getBattleScreen().getInformation().getIsBreak() || isLockedDuringAnimate();
        ...
        if (isDragging && (!controlsLocked)) {
            ...
            if (endDragSlot == -1) { // dragged to nothing
                ...
                if (startDragSlot <= 3) {
                    context.sendSelectMoveSlot(startDragSlot, MoveType.NONE);
                }
            }
                ...
                if (startDragSlot <= 3 && endDragSlot <= 3) { // drag from place to place; swap
                    ...
                    if (isBigShip) {
                        if ((blockingMoveSlot == startDragSlot) && endDragSlot <= 3) { // cant drag blockingMove off to new piece
                            ...
                            context.sendBlockingMoveSlotChanged(blockingMoveSlot);
                    ...
                    if ((!isBigShip) || (blockingMoveSlot != endDragSlot)) {
                        context.sendSelectMoveSlot(endDragSlot, MoveType.forId(startDragSlot-3));
                    }
            ...
            if (isTogglingAuto(x, y)) {
                ...
                if (
                    ...
                    context.sendGenerationTarget(targetMove);
                }
                else if (
